...
class Database:
    ...
    def get_day(self, gid, day, week):
        self.cursor.execute("""
            SELECT day, number, name, start_time, end_time, classroom, url
            FROM backend_schedule WHERE group_id = %s AND week_type = %s AND day = %s
        """, (gid, week, day.lower()))
        return self.cursor.fetchall()

    def get_uids_notif(self):
        self.cursor.execute("""SELECT user_id FROM backend_profiles WHERE notification = True""")
        return [key[0] for key in self.cursor.fetchall()]
